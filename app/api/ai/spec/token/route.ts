...
// POST /api/ai/spec/token - Issue a run-scoped Trigger.dev public token for spec generation.
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error("Spec task token creation failed", error)
    return errorResponse("Unable to create spec task token", 502)
  }
}
