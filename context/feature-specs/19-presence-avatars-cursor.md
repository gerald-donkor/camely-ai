...
4. Add live cursors to the canvas.
   ...
   - use the existing Liveblocks presence state to broadcast
     cursor position
   - use the Liveblocks React Flow cursor integration to update
     cursor position from React Flow pointer movement
   - broadcast cursor positions in React Flow canvas coordinates
     rather than viewport pixels
   - project cursor positions through each participant's local
     pan and zoom so both sides point to the same canvas location
